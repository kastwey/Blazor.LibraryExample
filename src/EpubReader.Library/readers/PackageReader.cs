...
namespace EpubReader.Library.Readers
{
	...
	{
		public static async Task<EpubPackage> ReadPackageAsync(ZipFile epubArchive, string rootFilePath)
		{
			var entryIndex = epubArchive.FindEntry(rootFilePath, true);
			if (entryIndex == -1)
			{
				throw new Exception("EPUB parsing error: root file not found in archive.");
			}
			var rootFileEntry = epubArchive[entryIndex];
			XDocument containerDocument;
			using (Stream containerStream = epubArchive.GetInputStream(rootFileEntry))
